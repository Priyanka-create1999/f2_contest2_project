...
// add all the object(students) into the table when application starts
...
function search(eventDetails){
    ...
    if(!Value){ // if search value is null, reset the table
        for(let student of arr){
            filler(student)
        }
        return
    ...
    // get all the students based on search input
...
// implementing button based sort
...
pass.addEventListener("click", () => {
    ...
    let passedStudents = arr.filter(student => student.passing)
    // empty table body
    while (tbody.firstChild) {
        tbody.removeChild(tbody.firstChild);
    }
    // fill table with passed students' data
    for(let student of passedStudents){
        filler(student)
    }
})
// implementing sort by gender
let countOfGenderSort = 1;
let gender = document.getElementById("gender")
gender.addEventListener("click", sortOnGender)

function sortOnGender(){
    // disable the button so that not more than one time male and female tables are created
    gender.disabled = true
    // change button's properties to indicate disability
    gender.style.backgroundColor = "gray"
    gender.style.cursor = "not-allowed"

    // get all male and female students separatly
    let maleStudents = arr.filter(student => student.gender.toLowerCase() === "male")
    let femaleStudents = arr.filter(student => student.gender.toLowerCase() === "female")
    // remove the original table
    let table = document.getElementsByClassName("custom-table")[0]
    table.remove()
    // create two tables with same properties
    let maleTable = document.createElement("table")
    let femaleTable = document.createElement("table")
    maleTable.className = "custom-table"
    maleTable.id = "male-table"
    let maleTableId = "male-table"
    femaleTable.className = "custom-table"
    femaleTable.id = "female-table"
    let femaleTableId = "female-table"


    maleTable.innerHTML = `<thead>
                            <tr>
                            <th>ID</th>
                            <th>Name</th>
                            <th>Gender</th>
    ...
    // append both tables into display area
    let display = document.getElementById("display")
    // create two headings
    let h1 = document.createElement("h3")
    let h2 = document.createElement("h3")
    h1.innerHTML = "Male Students"
    h2.innerHTML = "Female Students"
    // append in order to appearence
    display.append(h1)
    display.append(maleTable)
    display.append(h2)
    display.append(femaleTable)
    // fill tables with their respective data
    //1. filling male table with maleStudents
    fillTable(maleStudents, maleTableId)
    //2. filling female table with femaleStudents
    fillTable(femaleStudents, femaleTableId)
}

function fillTable(students, tableId){
    ...
    for(let student of students){
        ...
        // create cells into the row and add corresponding values
        ...
        // set the image in the name cell
        //1. get the image src
        let imgSrc = student.img_src
        //2. create a div to accomodate the image
        let imgDiv = document.createElement("div")
        imgDiv.className = "img-div"
        //3. set div properties and add image to div as background
        imgDiv.style.backgroundImage = `url(${imgSrc})`
        //3. append image to student-row in name column
        cellName.prepend(imgDiv)
    }
}
